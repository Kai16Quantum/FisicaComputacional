...
const int imprimir_cada_x_pasos = 15;
...
int main(){
    FILE *f_hamiltoniano;
    char nombre_estabilidad[64] = "Datos/datos_estabilidad";
        ...
        for (int l=0; l<max_energias; l++) {
            ...
            for (int k = 0; k<numero_de_pasos; k++) {
                ...
                if (k%imprimir_cada_x_pasos == 0){
                    fprintf(f_estabilidad, "%f,\t", t);
                    for (int i=0; i<2; i++) {
                        fprintf(f_estabilidad, "%f,\t", valores_iniciales[i]);
                    }
                    fprintf(f_estabilidad, "\n");
